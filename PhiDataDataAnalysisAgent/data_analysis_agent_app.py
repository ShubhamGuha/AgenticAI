...
# Initialize Pinecone and create the vector index if missing
...
def describe_data(filepath: str) -> str:
    """Return a statistical summary for the provided CSV or Excel file."""
    df = pd.read_csv(filepath) if filepath.endswith(".csv") else pd.read_excel(filepath)
    return df.describe().to_string()

@tool
def embed_and_store(filepath: str) -> str:
    """Embed a file summary and store the resulting vectors in Pinecone."""
    df = pd.read_csv(filepath) if filepath.endswith(".csv") else pd.read_excel(filepath)
...
def search_similar(query: str) -> str:
    """Return the top semantic matches for a metadata query."""
...
# User interface: upload files and show analysis results
